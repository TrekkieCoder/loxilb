...
RUN mkdir -p /opt/loxilb && \
    ...
    arch=$(arch | sed s/aarch64/arm64/ | sed s/x86_64/amd64/) && echo $arch && cpus=$(nproc) && \
    ...
    if [ "$arch" = "arm64" ] ; then apt purge -y gcc-arm-linux-gnueabihf; else apt-get update && apt purge -y gcc-multilib;fi && \
